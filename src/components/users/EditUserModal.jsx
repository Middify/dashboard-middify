...
const EditUserModal = ({
  ...
  const availableRoles = getRolesPermitidos(currentUser?.role);
  ...
  useEffect(() => {
    if (open && user) {
      ...
      const initialTenants = allTenants.filter((t) =>
        userTenantIds.has(t.tenantId),
      );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const hasTenantsChanged =
        tenantsToAssign.length > 0 || tenantsToRemove.length > 0;
      ...
      await updateUser({
        ...
        tenantsToAssign:
          tenantsToAssign.length > 0 ? tenantsToAssign : undefined,
        tenantsToRemove:
          tenantsToRemove.length > 0 ? tenantsToRemove : undefined,
      });
        ...
        {user && (
          <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 shadow-sm">
            <Typography
              variant="subtitle2"
              color="textSecondary"
              className="uppercase tracking-wider text-xs font-semibold mb-1"
            >
            ...
            renderOption={(props, option, { selected }) => {
              ...
              return (
                <li
                  key={key}
                  {...otherProps}
                  className={`${otherProps.className} hover:bg-slate-50`}
                >
                  <Checkbox
                    icon={icon}
