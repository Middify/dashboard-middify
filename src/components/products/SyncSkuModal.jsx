...
const SyncSkuModal = ({
  ...
  const handleSync = async () => {
    ...
    if (activeTab === 1) {
      skusToSync = parseManualSkus();
    } else {
      // TODO: Aquí iría la lógica para leer el Excel/CSV.
      // Por ahora, validamos que no esté vacío el archivo.
      if (!selectedFile) {
        toast.warning("Por favor selecciona un archivo");
        return;
      }
      toast.info(
        "La lectura de archivos está en desarrollo, probemos la carga manual primero.",
      );
      return;
    }

    if (skusToSync.length === 0) {
      toast.warning("No hay SKUs válidos para sincronizar");
      return;
    }
    ...
    try {
      ...
      toast.success("SKUs enviados a sincronizar correctamente");
      ...
      toast.error(error.message || "Ocurrió un error al sincronizar los SKUs");
    } finally {
      setIsSyncing(false);
    }
  };
        ...
        <div className="space-y-4">
          {activeTab === 0 ? (
            <>
              ...
              <div className="rounded-lg bg-blue-50 p-3">
                <p className="text-sm text-blue-700">
                  <strong>Formatos aceptados:</strong> Excel (.xlsx, .xls), CSV
                  (.csv) o JSON (.json)
                </p>
              ...
              >
                {!selectedFile ? (
                  <>
                    <CloudUploadIcon className="mx-auto mb-3 text-slate-400 text-[48px]" />
                ...
                <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
                  ...
                  <ul className="space-y-1 text-xs text-slate-600">
                    <li className="flex gap-2">
                      <span>•</span>
                      <span>
                        <strong>SKU</strong> o <strong>SKUSIMPLE</strong>:
                        Código del producto a sincronizar (requerido)
                      </span>
                    </li>
                  </ul>
                  <p className="mt-2 text-xs text-slate-500">
                    💡 La primera fila debe contener los encabezados. Solo se
                    requiere la columna SKU.
                  </p>
                </div>
              )}
            </>
            ...
            <>
              ...
              <div className="rounded-lg bg-blue-50 p-3">
                <p className="text-sm text-blue-700">
                  <strong>Instrucciones:</strong> Escribe los SKUs uno por
                  línea, o separados por comas o punto y coma.
                </p>
                <p className="mt-1 text-xs text-blue-600">
