...
const StockTab = ({ history }) => {
  ...
  if (!history || history.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-slate-400 bg-slate-50 rounded-2xl border border-dashed border-slate-300">
        <TrendingFlatIcon className="text-[48px] mb-2 opacity-50" />
  ...
  const getTrendIcon = (prev, current) => {
    ...
    if (diff > 0)
      return <TrendingUpIcon fontSize="small" className="text-emerald-500" />;
    if (diff < 0)
      return <TrendingDownIcon fontSize="small" className="text-red-500" />;
    ...
    <div className="space-y-6">
      <div className="flex justify-end">
        <ToggleButtonGroup
          ...
          size="small"
          className="bg-white [&_.MuiToggleButton-root]:border [&_.MuiToggleButton-root]:border-slate-200 [&_.MuiToggleButton-root]:px-2 [&_.MuiToggleButton-root]:py-0.5 [&_.MuiToggleButton-root]:normal-case [&_.MuiToggleButton-root]:font-semibold [&_.MuiToggleButton-root]:text-slate-600 [&_.MuiToggleButton-root.Mui-selected]:bg-blue-50 [&_.MuiToggleButton-root.Mui-selected]:text-blue-600 [&_.MuiToggleButton-root.Mui-selected]:border-blue-200 [&_.MuiToggleButton-root.Mui-selected:hover]:bg-blue-100"
        >
      ...
      {viewMode === "table" ? (
        ...
        >
          <Table className="min-w-[650px]" aria-label="historial stock table">
            <TableHead>
              <TableRow className="bg-slate-50">
                ...
                <TableCell
                  align="right"
                  className="font-bold text-slate-600 text-xs uppercase"
                >
                  Anterior
                </TableCell>
                <TableCell align="center" className="w-[50px]"></TableCell>
                <TableCell
                  align="left"
                  className="font-bold text-slate-600 text-xs uppercase"
                >
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="max-w-[200px] sm:max-w-[250px]">
                      <MuiTooltip title={log.operacion} placement="top" arrow>
                        <Chip
                          label={log.operacion}
                          size="small"
                          variant="outlined"
                          className={`font-bold max-w-full [&_.MuiChip-label]:truncate ${
                            log.operacion?.toLowerCase().includes("error")
                              ? "bg-red-50 border-red-200 text-red-700 uppercase text-[10px]"
                              : "capitalize bg-slate-50 border-slate-200 text-slate-700"
                          }`}
                        />
                      </MuiTooltip>
                    </TableCell>
                    ...
                    <TableCell align="left">
                      <span
                        className={`font-mono font-bold text-base ${stockDiff > 0 ? "text-emerald-600" : stockDiff < 0 ? "text-red-600" : "text-slate-700"}`}
                      >
                        {log.stockNuevo}
                      </span>
                    </TableCell>
