...
const MainLayout = () => {
  ...
  const resolvedOrderState = useMemo(
    () => ensureOrderState(searchParams.get("state")),
    [searchParams],
  );
  const resolvedProductState = useMemo(
    () => searchParams.get("productState"),
    [searchParams],
  );
  const resolvedPriceState = useMemo(
    () => searchParams.get("priceState"),
    [searchParams],
  );
  ...
  const currentView = useMemo(
    () => deriveView(location.pathname, Boolean(detailOrderId)),
  ...
  const isLoading =
    productLoading || marketplaceLoading || userLoading || authTenantsLoading;
  ...
  const selectedTenantName = useMemo(() => {
    ...
    return (
      (authorizedTenants || []).find((t) => t.tenantId === selectedTenantId)
        ?.tenantName ?? null
    );
  }, [selectedTenantId, authorizedTenants]);
    ...
    (nextView) => {
      switch (nextView) {
        ...
        case "users":
          ...
          break;
        case "users-list":
          navigate("/users?view=list");
          break;
        case "users-create":
          navigate("/users?view=create");
          break;
        case "orders": {
  ...
  const handleSelectProductState = useCallback(
    (stateId) => {
      navigate(
        stateId
          ? `/products?productState=${encodeURIComponent(stateId)}`
          : "/products",
      );
    },
    [navigate],
  ...
  const handleSelectPriceState = useCallback(
    (stateId) => {
      navigate(
        stateId ? `/price?priceState=${encodeURIComponent(stateId)}` : "/price",
      );
    },
    [navigate],
  );
